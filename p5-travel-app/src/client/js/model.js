  ...
  <div class="card mb-12">
    <div class="row no-gutters">
      <div class="col-md-12">
        <img src="${
          trip.image
        }" class="card-img" alt="Picture of Travel Destination">
      </div>
      ...
      <div class="col-md-12">
        <div class="card-body">
          ...
          <span class="trip_countdown">Your trip to <span style="color:crimson">${
            trip.city
          } </span> is <span style="color:crimson">${daysLeft}</span>  days away</span>
          <p>Trip date weather: <span style="color:crimson">${
            weather.temperature
          }&deg;F</span>, <span style="color:crimson">${
    weather.summary
  }</span> </p>
        </div>
      </div>
...
const showModal = (trip) => {
  ...
  document.querySelector(
    ".trip_countdown"
  ).innerText = `Your trip to ${trip.city} is ${daysLeft} days away`;

  // Display weather info
  const weather = getWeatherInfo(trip.weatherForecast, daysLeft, tripStart);
  document.querySelector(
    ".trip_weather"
  ).innerHTML = `<p>Trip date weather: <span style="color:crimson">${weather.temperature}&deg;F</span>, <span style="color:crimson">${weather.summary}</span> </p>`;
};
...
const getTripDate = (date) => {
  ...
  const tripDateText = `${days[tripDate.getDay()]}, ${
    months[tripDate.getMonth()]
  } ${tripDate.getDate()}, ${tripDate.getFullYear()}`;
